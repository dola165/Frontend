    ...
    if (mediaItems.length === 0) {
        return (
            <section className="rounded-[24px] border border-[color:var(--club-theme-border-subtle)] bg-[rgba(12,18,27,0.96)] px-5 py-14 text-center shadow-[0_18px_32px_rgba(2,6,12,0.22)]">
                ...
                <h3 className="mt-4 text-xl font-black tracking-[-0.03em] text-[color:var(--club-theme-text-primary)]">No media published yet</h3>
    ...
    return (
        <section className="rounded-[24px] border border-[color:var(--club-theme-border-subtle)] bg-[rgba(12,18,27,0.96)] p-5 shadow-[0_18px_32px_rgba(2,6,12,0.22)]">
            <div className="mb-5 flex items-end justify-between gap-4">
                <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[color:var(--club-tone-green)]">Media</p>
                    <h2 className="mt-2 text-2xl font-black tracking-[-0.04em] text-[color:var(--club-theme-text-primary)]">Club moments</h2>
                </div>
                <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[color:var(--club-theme-text-secondary)]">{mediaItems.length} assets</p>
            </div>
                    ...
                    return (
                        <article key={`${item.postId}-${index}`} className="group overflow-hidden rounded-[18px] border border-white/6 bg-white/[0.03]">
                            <div className="relative aspect-[4/3] overflow-hidden bg-black/20">
                                ...
                                {isVideo ? (
                                    <span className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-black/54 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-white">
