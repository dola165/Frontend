    ...
    if (!club) {
        return (
            <div className="bg-[#0f1117] flex h-full min-h-[calc(100vh-var(--app-header-height))] items-center justify-center px-6">
                <div className="rounded-md border border-[#ffffff0d] bg-[#16181d] px-8 py-10 text-center">
        ...
        <div className="bg-[#0f1117] min-h-full">
            <div className="border-b border-[#ffffff0d] px-6 py-5">
                ...
                <div className="mt-4 grid grid-cols-3 gap-3">
                    <div className="rounded-md border border-[#ffffff0d] bg-[#16181d] px-4 py-3">
            ...
            <div className="grid gap-6 px-6 py-6 xl:grid-cols-[280px_1fr_280px] xl:items-start">
                ...
                <div className="flex flex-col gap-4 xl:sticky xl:top-[calc(var(--app-header-height)+24px)]">
                    {/* Squad Navigation */}
                    <div className="rounded-md border border-[#ffffff0d] bg-[#16181d] overflow-hidden">
                                        ...
                                        <div key={squad.id}>
                                            {isEditing ? (
                                                <div className="space-y-2 px-4 py-3 bg-[var(--fc-accent-soft)]">
                                                    <input value={editForm.name ?? squad.name} onChange={(e) => setEditForm((f) => ({ ...f, name: e.target.value }))} placeholder="Squad name" className="w-full rounded-md border border-[var(--fc-border)] bg-[var(--fc-card-bg)] px-3 py-1.5 text-sm text-[#f4f4f5] outline-none" />
                                                    <input value={editForm.category ?? squad.category} onChange={(e) => setEditForm((f) => ({ ...f, category: e.target.value }))} placeholder="Category" className="w-full rounded-md border border-[var(--fc-border)] bg-[var(--fc-card-bg)] px-3 py-1.5 text-sm text-[#f4f4f5] outline-none" />
                                                    <select value={editForm.gender ?? squad.gender} onChange={(e) => setEditForm((f) => ({ ...f, gender: e.target.value }))} className="w-full rounded-md border border-[var(--fc-border)] bg-[var(--fc-card-bg)] px-3 py-1.5 text-sm text-[#f4f4f5] outline-none">
                                                        <option value="MALE">MALE</option><option value="FEMALE">FEMALE</option><option value="MIXED">MIXED</option>
                                                    </select>
                                                    <div className="flex gap-2">
                                                        <button type="button" onClick={() => void handleUpdateSquad(squad.id)} disabled={savingSquad} className="rounded-md bg-[#16a34a] px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90 disabled:opacity-50">{savingSquad ? 'Saving...' : 'Save'}</button>
                                                        <button type="button" onClick={() => { setEditingSquadId(null); setEditForm({}); }} className="rounded-md border border-[#ffffff0d] px-3 py-1.5 text-xs font-semibold text-[#a1a1aa] hover:text-[#f4f4f5]"><X className="h-3 w-3" /></button>
                                                    </div>
                                                </div>
                                            ) : (
                ...
                <div className="flex flex-col gap-4">
                    {squads.length === 0 ? (
                        <div className="rounded-md border border-[#ffffff0d] bg-[#16181d] p-5 text-center">
                            ...
                            {(isClubAdmin && selectedSquad) && (
                                <div className="flex justify-end gap-2">
                                    <button
                                        ...
                                        className="rounded-md border border-[#ffffff0d] px-3 py-1.5 text-xs font-semibold text-[#a1a1aa] hover:text-[#f4f4f5] transition-colors"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="rounded-md bg-[#16a34a] px-3 py-1.5 text-xs font-semibold text-white hover:opacity-90 transition-opacity"
                                    >
                                        + Add Players
                                    </button>
                                </div>
                ...
                <div className="flex flex-col gap-4 xl:sticky xl:top-[calc(var(--app-header-height)+24px)]">
                    ...
                    <div className="rounded-md border border-[#ffffff0d] bg-[#16181d] overflow-hidden">
                        ...
                        <div className="px-4 py-3">
                            <Link to={`/clubs/${club.id}`} className="inline-flex items-center justify-between gap-2 w-full rounded-md border border-[#ffffff0d] bg-[var(--fc-card-bg)] px-3 py-2 text-sm font-medium text-[#f4f4f5] hover:bg-[var(--fc-surface-hover)] transition-colors">
                                Back to Club
                                <ChevronLeft className="h-3.5 w-3.5" />
                            </Link>
                        </div>
